...
const { isWrappedId, WRAPPED_SUFFIX, REQUIREFUNC_ID } = require("./helpers")
...
function analyzeImport(node, importBindings, requireFuncBindings, code, getName, globals) {
  ...
  globals.add(name);
  const isWrappedSuffix = isWrappedId(id, WRAPPED_SUFFIX)
  for (const spec of node.specifiers) {
    if (isWrappedSuffix && spec.imported.name === REQUIREFUNC_ID) {
      requireFuncBindings.set(spec.local.name, name);
      continue;
    }
...
function writeCallExpression(code, node, name) {
...
async function importToGlobals({ ast, code, getName, getDynamicWrapper }) {
  ...
  const requireFuncBindings = new Map;
  ...
  for (const node of ast.body) {
    if (node.type === "ImportDeclaration") {
      isTouched = analyzeImport(node, bindings, requireFuncBindings, code, getName, globals) || isTouched;
  ...
  walk(ast, {
    enter(node, parent) {
      ...
      if (isReference(node, parent)) {
        ...
        } else if (requireFuncBindings.has(node.name) && !scope.contains(node.name)) {
          if (parent.type === "CallExpression" && parent.callee === node) {
            writeCallExpression(code, parent, requireFuncBindings.get(node.name));
            this.skip();
          }
