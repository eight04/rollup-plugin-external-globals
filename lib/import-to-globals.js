...
function writeDynamicImport(code, node, globalName, dynamicWrapper = "Promise.resolve") {
  code.overwrite(node.start, node.end, `${dynamicWrapper}(${globalName})`);
}
...
function importToGlobals({ast, code, getName, dynamicWrapper}) {
  ...
  walk(ast, {
    enter(node, parent) {
      ...
      if (name) {
        writeDynamicImport(code, node, name, dynamicWrapper);
        isTouched = true;
        this.skip();
      }
    },
